...
struct TimetableProvider: TimelineProvider {
    ...
    private func getDay(completionHandler: @escaping ((_ success: Bool, _ day: Day?) -> Void)) {
        guard let ps = EduLinkAPI.shared.defaults.value(forKey: "PreferredSchool") as? String, let pu = EduLinkAPI.shared.defaults.value(forKey: "PreferredUsername") as? String else { return completionHandler(false, nil) }
        ...
        if user == nil { return completionHandler(false, nil) }
...
struct CentralisWidgetEntryView : View {
        ...
        GeometryReader { reader in
            VStack(spacing: 10) {
                ForEach(entry.day?.periods ?? [Period](), id: \.self) { period in
                        HStack {
                            Text("\(period.start_time) - \(period.end_time) - \(period.lesson != nil ? period.lesson.room_name : "Free") - \(period.lesson != nil ? period.lesson.subject : "Free")")
                                .padding()
                ...
                .frame(width: reader.size.width * 0.85, height: (reader.size.height - (CGFloat(entry.day?.periods.count ?? 1) * 30) / CGFloat(entry.day?.periods.count ?? 1)), alignment: .center)
            }
...
struct CentralisWidget_Previews: PreviewProvider {
    static var previews: some View {
        CentralisWidgetEntryView(entry: TimetableEntry(date: Date(), day: DemoDay))
